...
| `tbl_funnel.csv` | Daily marketing funnel metrics | date, product_id, view, click, add_to_cart, purchase | ~3,000 rows (Jan 2024 – Sep 2025) |
| `tbl_transaction.csv` | Individual sales transactions | trx_id, product_id, trx_date, units | ~5,000+ transactions |
...
A preview of `f_summary_transaction` (top 3 rows):

| trx_date   | product_id   | product_name        | total_units | total_revenue | total_profit | total_purchase | total_add_to_cart | total_click | total_view |
|------------|--------------|---------------------|-------------|---------------|--------------|----------------|-------------------|-------------|------------|
| 2024-01-01 | DQProduk-001 | Buku Agenda DQLab   | 87          | 20,866,950    | 7,830,000    | 87             | 979              | 3434        | 6631       |
| 2024-01-01 | DQProduk-003 | Sticky Notes DQLab  | 9           | 538,650       | 269,550      | 9              | 90               | 353         | 535        |
| 2024-01-01 | DQProduk-009 | Pulpen Metal DQLab  | 98          | 16,155,300    | 1,470,000    | 98             | 927              | 4573        | 8043       |

> **Full output** can be regenerated by running `script/clean_integration.sql` and then `SELECT * FROM f_summary_transaction`.
